...
const GestionListePrincipale = () => {
  const { inscriptions } = useInscriptions();
  const items = inscriptions.filter((i) => i.listType === "principale");
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }}>
        <h1 className="text-2xl lg:text-3xl font-display font-bold">Liste principale</h1>
        <p className="text-muted-foreground mt-1">Enfants titulaires inscrits en liste principale.</p>
      </motion.div>
        ...
        <Card className="glass-card">
          <CardHeader className="pb-3 flex flex-row items-center justify-between">
            <CardTitle className="font-display text-lg flex items-center gap-3">
              <Badge className="bg-success/10 text-success border-success/20 px-3 py-1 text-sm font-semibold">
                <CheckSquare className="w-4 h-4 mr-1.5" />
                Liste principale
              </Badge>
              <span className="text-sm font-normal text-muted-foreground">Enfants titulaires</span>
              <Badge variant="outline" className="ml-2">{items.length}</Badge>
            </CardTitle>
            <div className="flex gap-2">
              <Button size="sm" variant="outline" onClick={() => exportToPDF(exportData, "Liste principale", ["Matricule", "Parent", "Service", "Enfant", "Âge", "Sexe", "Statut"])}>
                <Download className="w-3.5 h-3.5 mr-1.5" /> PDF
              </Button>
              <Button size="sm" variant="outline" onClick={() => exportToExcel(exportData, "Liste_principale")}>
...
export default GestionListePrincipale;
